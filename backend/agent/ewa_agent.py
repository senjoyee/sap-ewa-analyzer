...
3.  **Executive Summary**: Write a concise summary for a C-level audience, highlighting the overall system status, key business risks, and the most critical actions required. Make it bulleted.
...
10. **Parameters Table**: Extract and list all relevant parameters across the stack (App & DB) identified in the report. For each parameter, provide: name, area, current value, recommended value & description. Present this as an array in the JSON for tabular display. MAKE SURE ALL PARAMETERS ACROSS THE DOCUMENT ARE CAPTURED.
