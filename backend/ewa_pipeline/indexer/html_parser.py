"""
HTML to Markdown parser for SAP EWA reports.
...
- Pre-computed hash table for instant lookup of known icons
...
Public API (mirrors pdf_parser conventions):
    parse_html_to_markdown(html_path, output_dir) -> tuple[str, Path]
    convert_html_to_markdown(html_path, output_path=None) -> str   # lower-level
"""
...
def _normalize_report_stem(path: Path | str) -> str:
    """Return the report stem without Word HTML suffixes like _files."""
...
# Pre-computed MD5 hash table covering all known EWA rating icons.
...
def build_icon_map(images_dir: str) -> dict[str, str]:
    """Pre-scan all GIFs/PNGs in the companion folder and build {filename: label}."""
...
def convert_html_to_markdown(
    ...
    # Locate companion images folder (Word naming: filename_files/ or filename.files/)
...
def parse_html_to_markdown(html_path: Path, output_dir: Path) -> tuple[str, Path]:
    """
    Convert an EWA HTML file to a single markdown file.

    Mirrors parse_pdf_to_markdown(pdf_path, output_dir) -> tuple[str, Path].
    The output is saved as output_dir/{html_stem}.md.

    Returns:
        (markdown_text, saved_md_path)
    """
    md_path = output_dir / f"{_normalize_report_stem(html_path)}.md"
    markdown = convert_html_to_markdown(str(html_path), output_path=str(md_path))
